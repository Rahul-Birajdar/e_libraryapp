...
const BookDetails = () => {
  ...
  useEffect(() => {
    ...
    const fetchDetails = async () => {
      ...
      try {
        ...
        }&maxResults=12&startIndex=${startIndex}`; // Include the startIndex in the query

        const response = await axios.get(
          `https://www.googleapis.com/books/v1/volumes?${query}`
        );
        setDetails((prevDetails) => [...prevDetails, ...response.data.items]); // Append new books to the existing list
        setIsLoading(false);
      } catch (error) {
    ...
    fetchDetails();
  }, [term, author, category, startIndex]); // Include startIndex in the dependencies array
  ...
  const loadMore = async () => {
    try {
      setStartIndex((prevIndex) => prevIndex + 12); // Increment the start index by 12 for the next page
    } catch (error) {
